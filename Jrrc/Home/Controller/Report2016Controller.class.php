...
class Report2016Controller extends Controller {
	...
	public function show_Unit_Report($start, $end) {
		...
		foreach ( $report as $r ) {
			if ($r ['Head_office'] == '08100') {
				...
				$branch_sum ['task_amount_js_sum_private'] = $branch_sum ['task_amount_js_sum_private'] + $r ['task_amount_js_private'];
				$branch_sum ['task_amount_js_sum_company'] = $branch_sum ['task_amount_js_sum_company'] + $r ['task_amount_js_company'];
				...
				$branch_sum ['amount_js_sum_private'] = $branch_sum ['amount_js_sum_private'] + $r ['amount_js_private'];
				$branch_sum ['amount_js_sum_company'] = $branch_sum ['amount_js_sum_company'] + $r ['amount_js_company'];
				...
				$branch_sum ['js_private_complete_rate_sum'] = $branch_sum ['amount_js_sum_private'] / $branch_sum ['task_amount_js_sum_private'] * 100;
				$branch_sum ['js_company_complete_rate_sum'] = $branch_sum ['amount_js_sum_company'] / $branch_sum ['task_amount_js_sum_company'] * 100;
				...
				$branch_sum ['amount_js_season_sum_private'] = $branch_sum ['amount_js_season_sum_private'] + $r ['season_amount_js_private'];
				$branch_sum ['amount_js_season_sum_company'] = $branch_sum ['amount_js_season_sum_company'] + $r ['season_amount_js_company'];
				...
				$department_sum ['task_amount_js_sum_private'] = $department_sum ['task_amount_js_sum_private'] + $r ['task_amount_js_private'];
				$department_sum ['task_amount_js_sum_company'] = $department_sum ['task_amount_js_sum_company'] + $r ['task_amount_js_company'];
				...
				$department_sum ['amount_js_sum_private'] = $department_sum ['amount_js_sum_private'] + $r ['amount_js_private'];
				$department_sum ['amount_js_sum_company'] = $department_sum ['amount_js_sum_company'] + $r ['amount_js_company'];
				...
				$department_sum ['js_private_complete_rate_sum'] = $department_sum ['amount_js_sum_private'] / $department_sum ['task_amount_js_sum_private'] * 100;
				$department_sum ['js_company_complete_rate_sum'] = $department_sum ['amount_js_sum_company'] / $department_sum ['task_amount_js_sum_company'] * 100;
				...
				$department_sum ['amount_js_season_sum_private'] = $department_sum ['amount_js_season_sum_private'] + $r ['season_amount_js_private'];
				$department_sum ['amount_js_season_sum_company'] = $department_sum ['amount_js_season_sum_company'] + $r ['season_amount_js_company'];
		...
		$total = array ();
		$total ['task_amount_js_sum'] = $branch_sum ['task_amount_js_sum'] + $department_sum ['task_amount_js_sum'];
		$total ['amount_js_sum'] = $branch_sum ['amount_js_sum'] + $department_sum ['amount_js_sum'];
		$total ['amount_js_sum_private'] = $branch_sum ['amount_js_sum_private'] + $department_sum ['amount_js_sum_private'];
		$total ['amount_js_sum_company'] = $branch_sum ['amount_js_sum_company'] + $department_sum ['amount_js_sum_company'];
		...
		$total ['task_amount_cx_sum'] = $branch_sum ['task_amount_cx_sum'] + $department_sum ['task_amount_cx_sum'];
		
		$total ['js_complete_rate_sum'] = $total ['amount_js_sum'] / $total ['task_amount_js_sum'] * 100;
		$total ['js_private_complete_rate_sum'] = $total ['amount_js_sum_private'] / $branch_sum ['task_amount_js_sum_private'] * 100;
		$total ['js_company_complete_rate_sum'] = $total ['amount_js_sum_company'] / $branch_sum ['task_amount_js_sum_company'] * 100;
	...
	public function Unit_Report($start, $end) {
		...
		foreach ( $Unit_list as $key => $v ) {
			...
			$task_amount_js_private = $this->get_unit_task ( $v ['br_id'], $end, '5' );
			$task_amount_js_company = $this->get_unit_task ( $v ['br_id'], $end, '6' );
			...
			// 取得业务实绩
			$amount_js_private = $this->get_unit_js_type ( $v ['br_id'], $start, $end, '2' );
			$amount_js_company = $this->get_unit_js_type ( $v ['br_id'], $start, $end, '1' );
			$amount_js = $amount_js_private + $amount_js_company;
			...
			$season_amount_js_company = $this->get_unit_js_type ( $v ['br_id'], $startday, $endday, '1' );
			$season_amount_js = $season_amount_js_private + $season_amount_js_company;
			...
			$Report [$key] ['task_amount_js_private'] = $task_amount_js_private;
			$Report [$key] ['task_amount_js_company'] = $task_amount_js_company;
			...
			$Report [$key] ['amount_js_private'] = $amount_js_private / 10000;
			...
			$Report [$key] ['js_private_complete_rate'] = $amount_js_private / 10000 / $task_amount_js_private * 100;
			...
			$Report [$key] ['season_amount_js_private'] = $season_amount_js_private / 10000;
			$Report [$key] ['season_amount_js_company'] = $season_amount_js_company / 10000;
	...
	/**
	 * 取得所有的经营单位的对公国际结算量\对私国际结算量
	 */
	public function get_unit_js_type($unit, $start, $end, $er_type) {
		$model = M ( "ywls_fixed" );
		// 客户类型ER_TYPE=1 对公客户
		// 客户类型ER_TYPE=2对私客户
		$condition ['er_type'] = $er_type;
		$condition ['upbranch'] = $unit;
		...
		// 对私客户,除了个人外汇汇款业务,还包含21046个人外币现钞兑换
		if ($er_type == '2') {
			$condition ['_string'] = "
					( type='01' and yw_type not in('lr','la','ib'))
					or 
					( type='02' and yw_type in ('jh99','sh99'))					
					";
		}
	...
	public function Export_Unit_Excel($start, $end) {
		...
		$objPHPExcel->getActiveSheet ()->mergeCells ( 'q1:t2' );
		$objPHPExcel->getActiveSheet ()->mergeCells ( 'u1:x2' );
		...
		$objPHPExcel->getActiveSheet ()->setCellValue ( 'q1', '对私国际结算量' );
		$objPHPExcel->getActiveSheet ()->setCellValue ( 'u1', '对公国际结算量' );
		...
		$objPHPExcel->getActiveSheet ()->setCellValue ( 'q3', '任务' );
		$objPHPExcel->getActiveSheet ()->setCellValue ( 'r3', '实绩' );
		$objPHPExcel->getActiveSheet ()->setCellValue ( 's3', '完成率（%）' );
		$objPHPExcel->getActiveSheet ()->setCellValue ( 't3', '本季业绩' );
		
		$objPHPExcel->getActiveSheet ()->setCellValue ( 'u3', '任务' );
		$objPHPExcel->getActiveSheet ()->setCellValue ( 'v3', '实绩' );
		$objPHPExcel->getActiveSheet ()->setCellValue ( 'w3', '完成率（%）' );
		$objPHPExcel->getActiveSheet ()->setCellValue ( 'x3', '本季业绩' );
		...
		foreach ( $result as $u ) {
			if ($u ['Head_office'] == '08100') {
				...
				$objPHPExcel->getActiveSheet ()->setCellValue ( 'q' . $row, $u ['task_amount_js_private'] );
				$objPHPExcel->getActiveSheet ()->setCellValue ( 'r' . $row, $u ['amount_js_private'] );
				$objPHPExcel->getActiveSheet ()->setCellValue ( 's' . $row, $u ['amount_js_private'] / $u ['task_amount_js_private'] * 100 );
				$objPHPExcel->getActiveSheet ()->setCellValue ( 't' . $row, $u ['season_amount_js_private'] );
				
				$objPHPExcel->getActiveSheet ()->setCellValue ( 'u' . $row, $u ['task_amount_js_company'] );
				$objPHPExcel->getActiveSheet ()->setCellValue ( 'v' . $row, $u ['amount_js_company'] );
				$objPHPExcel->getActiveSheet ()->setCellValue ( 'w' . $row, $u ['amount_js_company'] / $u ['task_amount_js_company'] * 100 );
				$objPHPExcel->getActiveSheet ()->setCellValue ( 'x' . $row, $u ['season_amount_js_company'] );
		...
		$objPHPExcel->getActiveSheet ()->setCellValue ( 'q' . $row, "=sum(q4:q" . ($row - 1) . ")" );
		$objPHPExcel->getActiveSheet ()->setCellValue ( 'r' . $row, "=sum(r4:r" . ($row - 1) . ")" );
		$objPHPExcel->getActiveSheet ()->setCellValue ( 's' . $row, "=r" . $row . "/q" . $row . "*100" );
		$objPHPExcel->getActiveSheet ()->setCellValue ( 't' . $row, "=sum(t4:t" . ($row - 1) . ")" );
		
		$objPHPExcel->getActiveSheet ()->setCellValue ( 'u' . $row, "=sum(u4:u" . ($row - 1) . ")" );
		$objPHPExcel->getActiveSheet ()->setCellValue ( 'v' . $row, "=sum(v4:v" . ($row - 1) . ")" );
		$objPHPExcel->getActiveSheet ()->setCellValue ( 'w' . $row, "=v" . $row . "/u" . $row . "*100" );
		$objPHPExcel->getActiveSheet ()->setCellValue ( 'x' . $row, "=sum(x4:x" . ($row - 1) . ")" );
		...
		$objPHPExcel->getActiveSheet ()->setCellValue ( 'q' . $row, "=sum(q" . $department_row . ":q" . ($row - 1) . ")" );
		$objPHPExcel->getActiveSheet ()->setCellValue ( 'r' . $row, "=sum(r" . $department_row . ":r" . ($row - 1) . ")" );
		$objPHPExcel->getActiveSheet ()->setCellValue ( 's' . $row, "=r" . $row . "/q" . $row . "*100" );
		$objPHPExcel->getActiveSheet ()->setCellValue ( 't' . $row, "=sum(t" . $department_row . ":t" . ($row - 1) . ")" );
		
		$objPHPExcel->getActiveSheet ()->setCellValue ( 'u' . $row, "=sum(u" . $department_row . ":u" . ($row - 1) . ")" );
		$objPHPExcel->getActiveSheet ()->setCellValue ( 'v' . $row, "=sum(v" . $department_row . ":v" . ($row - 1) . ")" );
		$objPHPExcel->getActiveSheet ()->setCellValue ( 'w' . $row, "=v" . $row . "/u" . $row . "*100" );
		$objPHPExcel->getActiveSheet ()->setCellValue ( 'x' . $row, "=sum(x" . $department_row . ":x" . ($row - 1) . ")" );
		...
		$objPHPExcel->getActiveSheet ()->setCellValue ( 'q' . $row, "=q" . $total_branch_row . "+q" . $total_depart_row );
		$objPHPExcel->getActiveSheet ()->setCellValue ( 'r' . $row, "=r" . $total_branch_row . "+r" . $total_depart_row );
		$objPHPExcel->getActiveSheet ()->setCellValue ( 's' . $row, "=r" . $row . "/q" . $row . "*100" );
		$objPHPExcel->getActiveSheet ()->setCellValue ( 't' . $row, "=t" . $total_branch_row . "+t" . $total_depart_row );
		
		$objPHPExcel->getActiveSheet ()->setCellValue ( 'u' . $row, "=u" . $total_branch_row . "+u" . $total_depart_row );
		$objPHPExcel->getActiveSheet ()->setCellValue ( 'v' . $row, "=v" . $total_branch_row . "+v" . $total_depart_row );
		$objPHPExcel->getActiveSheet ()->setCellValue ( 'w' . $row, "=v" . $row . "/u" . $row . "*100" );
		$objPHPExcel->getActiveSheet ()->setCellValue ( 'x' . $row, "=x" . $total_branch_row . "+x" . $total_depart_row );
		...
		$objPHPExcel->getActiveSheet ()->getStyle ( 'q4:q' . $row )->getNumberFormat ()->setFormatCode ( '#,##' );
		$objPHPExcel->getActiveSheet ()->getStyle ( 'r4:r' . $row )->getNumberFormat ()->setFormatCode ( '#,##0.00' );
		$objPHPExcel->getActiveSheet ()->getStyle ( 's4:s' . $row )->getNumberFormat ()->setFormatCode ( '#,##0.00' );
		$objPHPExcel->getActiveSheet ()->getStyle ( 't4:t' . $row )->getNumberFormat ()->setFormatCode ( '#,##0.00' );
		
		$objPHPExcel->getActiveSheet ()->getStyle ( 'u4:u' . $row )->getNumberFormat ()->setFormatCode ( '#,##' );
		$objPHPExcel->getActiveSheet ()->getStyle ( 'v4:v' . $row )->getNumberFormat ()->setFormatCode ( '#,##0.00' );
		$objPHPExcel->getActiveSheet ()->getStyle ( 'w4:w' . $row )->getNumberFormat ()->setFormatCode ( '#,##0.00' );
		$objPHPExcel->getActiveSheet ()->getStyle ( 'x4:x' . $row )->getNumberFormat ()->setFormatCode ( '#,##0.00' );
		...
		$objPHPExcel->getActiveSheet ()->getStyle ( "a1:x" . $row )->applyFromArray ( $styleArray );
		...
		$objPHPExcel->getActiveSheet ()->getStyle ( 'a1:x' . ($row) )->applyFromArray ( $styleArray1 );
	...
	public function Export_Client_first_date($start, $end) {
		...
		$objPHPExcel->setActiveSheetIndex ( 0 )->setCellValue ( 'A1', '行号' )->setCellValue ( 'B1', '行名' )->setCellValue ( 'C1', '客户名称' )->setCellValue ( 'D1', '首笔业务日期' );
		// Add some data
		$row = 2;
		foreach ( $client_list as $v ) {
			
			$objPHPExcel->setActiveSheetIndex ( 0 )->setCellValue ( 'A' . $row, " " . $v ['upbranch'] )->setCellValue ( 'B' . $row, $v ['br_name'] )->setCellValue ( 'C' . $row, $v ['name'] )->setCellValue ( 'D' . $row, $v ['yw_date'] );
